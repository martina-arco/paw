...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    watch: {
      ...
      js: {
        files: ['<%= yeoman.app %>/scripts/**/*.js', '<%= yeoman.app %>/assets/**/*.js'],
    ...
    wiredep: {
      serve: {
        src: ['<%= yeoman.app %>/assets/main.scss'],
        ignorePath: /(\.\.\/){1,2}bower_components\//
      },
      dist: {
        src: ['.tmp/assets/main.scss'],
        ignorePath: /(\.\.\/){1,2}bower_components\//
      },
      options: {
        fileTypes: {
    ...
    filerev: {
      dist: {
        src: [
          ...
          '!<%= yeoman.dist %>/assets/img/static/*.jpg'
        ]
      },
      paths: {
    ...
    usemin: {
      ...
      imagesAndViews: ['<%= yeoman.dist %>/scripts/**/*.js', '<%= yeoman.dist %>/assets/**/*.js', '<%= yeoman.dist %>/views/**/*.html'],
      options: {
        assetsDirs: ['<%= yeoman.dist %>', '<%= yeoman.dist %>/assets/img'],
        patterns: {
          imagesAndViews: [
            [/(assets\/img\/.*?\.(?:gif|jpeg|jpg|png|webp))/gm, 'Update the JS to reference our revved images'],
    ...
    copy: {
      ...
      dist: {
        files: [{
          ...
          flatten: true,
          cwd: '.',
          src: 'bower_components/ng-dialog/css/*',
          dest: '<%= yeoman.dist %>/assets/css'
        }]
      },
      serve: {
  ...
  grunt.registerMultiTask('jsrev', 'Use filerev output to create require-js compatible path mappings', function () {
    ...
    for (var longModule in assets) {
      ...
      if (assets.hasOwnProperty(longModule)) {
          ...
          var shortPath = path.relative(options.baseRoot, longPath).replace('\\', '/');
          var shortModule = path.relative(options.baseRoot, longModule).replace('\\', '/');
